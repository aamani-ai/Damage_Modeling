# tropical_cyclone_wind_wind damage-code metadata spec — model v1.0/docs r1
...
curve_artifact: tropical_cyclone_wind_wind__model_v1_0__docs_r1__curve_artifact.json
capability: tropical_cyclone_wind_wind__model_v1_0__docs_r1__capability.json
...
The identity fields are atomic: release state and model grade are not embedded in the model version string.
This spec describes the repository-canonical partial-screening release. A production consumer may load it
only through the exact model/docs/schema/full-SHA pin and must preserve every declared withholding boundary.
...
`failure_unit_id` is optional in the reference helper so one call can return the full declared unit set:
...
No bridge is approved by this damage model. A bridge may be carried for evaluation only after
...
axis and no yawing. Because those statements are not fully consistent, the release does not translate them
...
| `scenario_loss_usd` with the source proxy | withheld `SOURCE_DENOMINATOR_CROSSWALK_NOT_APPROVED` |
...
| source-proxy scenario loss | withheld `SOURCE_DENOMINATOR_CROSSWALK_NOT_APPROVED` |
| EAL/PML/VaR/TVaR | withheld; downstream-owned and unsupported by this partial release |
...
- a production consumer cannot verify the exact current index entry and full artifact/capability/KAT/schema pins.

## 14. Capability declaration

Authoritative standalone current declaration:
...
annual metrics remain withheld by this partial release. Completeness of request fields cannot broaden capability.
...
3. `canonical_runtime_artifact=true` and exact equality with the canonical artifact index;
...
curve, the v0.1 no-curve scaffold under a new label, or an unpinned copy of the function.

## 16. Version and release rule

This is a `NEW_CELL_MODEL_RELEASE` plus `MODEL_BEHAVIOR_CHANGE` and `SCHEMA_CONTRACT_CHANGE`. The reviewed
partial-screening release is present in the canonical artifact index/current pointer and is authorized only
for source-native scalar turbine/tower screening. Scenario dollars, full-farm loss, and annual/tail metrics
remain withheld. Any widening of selectors, denominator, failure-unit coverage, axis bridge, range, or output
capability requires a newly classified and versioned change with updated pins, KATs, and rollback evidence.
