# Tropical-cyclone wind x onshore wind derivation dossier — proposed model v1.0/docs r1
...
This dossier supports a noncanonical numeric proposal. It does not publish the artifact, create a current
pointer, change Hazard, or authorize reportable portfolio metrics.
...
- [v0.1 source register](SOURCE_REGISTER_tropical_cyclone_wind_wind__model_v0_1__docs_r1.csv)
- [v0.1 claim/parameter register](CLAIM_PARAMETER_REGISTER_tropical_cyclone_wind_wind__model_v0_1__docs_r1.csv)
- [bounded evidence search](BOUNDED_EVIDENCE_SEARCH_LOG_tropical_cyclone_wind_wind__model_v0_1__docs_r1.md)
- [candidate formula audit](NUMERICAL_CANDIDATE_AUDIT_tropical_cyclone_wind_wind__model_v0_1__docs_r1.md)
- [legacy ingestion/reproduction](LEGACY_EVIDENCE_INGESTION_tropical_cyclone_wind_wind__model_v0_1__docs_r1.md)
- [pressure test](PRESSURE_TEST_tropical_cyclone_wind_wind__model_v0_1__docs_r1.md)
- [value crosswalk](VALUE_CROSSWALK_tropical_cyclone_wind_wind__model_v0_1__docs_r1.csv)
...
cannot produce scenario loss while the proposal is noncanonical.
...
Model v0.1 always returned null. The v1 proposal returns a scalar expected DR only for the source unit, exact
...
Existing canonical bundle-v2 artifacts are unchanged. A consumer that does not understand the proposed
...
before promotion; after promotion they would still require an approved denominator/value binding, occurrence
model, turbine exposure, dependency/cap logic, and exact pins. This proposal does not satisfy those
prerequisites.

## 18. Validation plan

The proposal is not review-ready until all of the following pass together:
...
9. canonical-index exclusion and current-runtime preservation checks;
10. Hazard dual-read fixture, exact model/docs/schema/SHA pin, and rollback rehearsal.

Passing repository-local checks establishes a coherent proposal. It does not itself promote the model.

## 19. Promotion and update triggers

Before any future canonical promotion:

- freeze and hash the final artifact/capability/KAT/workbook set;
- resolve or explicitly approve the source-unit reportability boundary;
- prove a real consumer fixture with exact source-axis metadata and supported selector;
- preserve nulls for all unsupported turbine/BOP units through Hazard M4;
- demonstrate no hardcoded consumer curve or legacy fallback;
- record an explicit reviewed promotion decision, index/current pointer, changelog, registry update, and
  rollback pin.
