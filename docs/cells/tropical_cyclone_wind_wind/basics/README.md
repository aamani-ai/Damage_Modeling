...
**Start here.** Model v1.2 is a canonical tower-only screening model for the shared 20×5 MW Wind Farm. It
uses one explicit owner-approved proxy; it is not target-matched or component-complete.

```yaml
cell_id: tropical_cyclone_wind_wind
cell_model_version: model v1.2
...
4. **Coverage is visibly partial.** Tower value covers 0.16 of project TIV. The other 0.84 is withheld,
   not zero.
...
named 5 MW target → unchanged Jaimes 3.3 MW response
             ...
             ▼
mean node DR × tower value (0.16 of project TIV)
             │
             ├── covered loss, capped at 0.16 of TIV
             └── remaining 0.84 withheld and disclosed
```
...
and must show both percent of the `$22.4M` tower value and percent of the `$140M` full TIV. It is not a
bankable estimate, field/claims calibration, or statement that the uncovered 84% is immune.
