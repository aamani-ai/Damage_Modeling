...
This addendum does not change wind/tornado × wind-farm model behavior. It makes the hub-height axis and runtime gates enforceable.
...
```text
source spine:
    IEC design-speed normalization and NIST-style fragility/damage-matrix framing.

chosen y-axis:
    failure-unit damage ratio for repeated-unit turbine structural bundles.

chosen form:
    bounded logistic on V_3s_hub / Ve50, with tornado as a direct-hit D50 shift rather than
    a separate claims-calibrated tornado curve.

sources demoted:
    tornado case studies and EF-scale sources are high-severity/context evidence, not full
    measured turbine fragility curves.
```
...
|---|---|---|---|
| `Ve50 = 1.4 × Vref` and IEC class table | `axis_bridge` | `T2_public_lab_standard_or_physics` | Standards/method bridge. |
| 10m→hub-height bridge | `axis_bridge` | `T3_engineering_proxy_or_adjacent_empirical` | Required by axis consistency; exponent/roughness must be site documented. |
| Structural D50 ratios and k | `curve_fit_shape` | `T4_placeholder_or_expert_judgment` | Public evidence supports form/mechanism, not exact failure probabilities. |
| Tornado D50 shifts | `conditioner_adjustment` | `T4_placeholder_or_expert_judgment` | Direct-hit behavior is plausible but not calibrated. |
