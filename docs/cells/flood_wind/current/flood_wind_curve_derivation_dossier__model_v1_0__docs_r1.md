> **Release overlay (2026-08-08).** This evidence narrative was assembled during proposal review. The curve/evidence discussion remains controlling, while proposal-status and future-promotion statements are historical. The canonical status, limitations, and consumer rules are governed by the sibling current JSON artifact, capability declaration, KATs, and release decision.

# Flood × wind curve derivation dossier — current model v1.0 / docs r1

> Released, canonical partial screening, and source-native. This dossier derives one whole-substation screening curve from
...
acquired or reviewed for this package. Successor review remains a model-improvement trigger and may refine
...
The model requires a pathway-aware piecewise-linear record. Released bundle v3 carries the governed
`piecewise_linear` payload; emit v2 carries `pathway_id`, and capability v3 enumerates the one conditional
unit and every withheld unit. The release does not silently alter bundle-v2 consumers.
...
- explicit historical status for NEMA GD 1-2016 and an improvement trigger for NEMA CS 70006-2026 review;
...
- current/index/SHA coherence plus shared-loader KAT execution.

## 14. Release boundary

This dossier supports a canonical model-v1 partial-screening release. Exact schema/evaluator/KAT validation
and the atomic model/docs/schema/SHA decision are complete. Independent field/claims calibration, current-
guide review, fuller unit coverage, and annual/tail consumer validation remain future improvements; same-
substation value and exposure checks remain mandatory at runtime.
