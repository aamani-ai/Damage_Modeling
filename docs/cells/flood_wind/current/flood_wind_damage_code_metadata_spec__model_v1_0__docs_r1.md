...
# Flood × wind damage-code metadata specification — current model v1.0 / docs r1

## Contract status

This specification governs the canonical pathway-aware partial-screening release identified by
...
whole-wind-farm completion, implicit value binding, annual/tail outputs, or automatic registry activation.
...
lineage remain mandatory consumer-side controls and may not be inferred from a numerical result.
...
as a model-improvement item. That review may revise conditioner or disposition policy but does not silently alter the
FEMA curve knots.
...
The evaluator emits DR; the common consumer may compute scenario loss only from:
