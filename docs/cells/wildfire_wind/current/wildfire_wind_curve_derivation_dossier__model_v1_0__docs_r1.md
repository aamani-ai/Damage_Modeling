...
contains only a mixed 72 USD/kW electrical row, so it cannot value either curve. The evaluator emits DR.
Scenario dollars may be computed only from an explicit same-unit direct replacement value and exposure
fraction. Aggregate electrical DR, whole-plant DR, EAL, PML, VaR, and TVaR are withheld.
...
The machine artifact, source/claim/tier/value registers, KATs, and workbook carry the exact current release. The
