...
**Why the damage curve is becoming its own repo, the three-phase arc it sits in, and the contract that
...
[`damage_curve_foundations/`](damage_curve_foundations/README.md) (principles + the six question-docs) and
the [`damage_curve_implementation/`](damage_curve_implementation/) library (the global method + three worked
cells).

> **Status — 🟡 architecture agreed, spin-out pending.** Method is mature (3 principles, ~17 method
> standards, 3 worked cells); evidence is young (public-derived, not claims-calibrated). Next big step: stand
> up a **separate `damage_modeling` repo** alongside hazard modeling and relocate this whole section into it.
...
```
   ...
   2. HAZARD  (this repo)  ── M0→M1→M2→[M3]→M4 ◄────┤   per failure-unit,        │
...
[`damage_curve_implementation/.../13_end_to_end_damage_work_architecture.md`](damage_curve_implementation/).
...
- **The artifact format:** today the curve records live in Excel — **temporary.** The canonical artifact
  becomes a **machine-readable JSON** (a direct serialization of the foundations'
  [`00_assembled_curve_record`](damage_curve_foundations/00_assembled_curve_record.md) schema), with Excel
  demoted to a derivation/view aid. The old `infrasure-damage-curves` repo's `master_curve_index.json` is the
  architectural reference for *how* to store this. (Deferred — for when the repo is stable.)
...
**The bridge is standard [16 · reference ingestion](damage_curve_implementation/).** It is the protocol to
...
```
   NOW                                                                          LATER
   ───────────────────────────────────────────────────────────────────────────────────►
   [1] write this scope & story        (you are reading it)
   [2] create the damage_modeling repo skeleton  (AGENTS.md, CLAUDE.md, README, .env, data/, notebooks/)
   [3] RELOCATE this whole section → new_repo/docs/   (move, not copy — one home, no drift)
   [4] run standard 16: harvest the old repo's evidence into the cells (close the gap)
   [5] machine-readable curve artifact (JSON from the assembled-curve-record schema)
   [6] hazard M3 consumes the published damage_code() via the versioned contract
   [7] (only then) the formal repo SPLIT is "done"
```

| Step | What | When / precondition |
|---|---|---|
| 1 | This doc | now |
| 2 | New repo skeleton, **alongside** hazard modeling | next big step |
| 3 | Move `damage_curves/` → `new_repo/docs/` | with step 2 |
| 4 | Evidence harvest via standard 16 | the real bottleneck; ongoing |
| 5 | JSON curve artifact (Excel → machine-readable) | when the repo is stable |
| 6 | Notebook M3 integration via the contract | after 5 |
| 7 | Treat the split as complete | **only** once the contract is defined **and** ≥1 consumer integration is proven |

**Anti-drift rule:** the damage section has **one** home. Once it moves to the new repo (step 3), it is *not*
maintained here too. This staging folder empties.

---

## 10 · Status, parked, open

- **Status:** 🟡 architecture agreed; 3 cells at model v1.0 (public-derived); spin-out is the next step.
...
*Links:* [`damage_curve_foundations/`](damage_curve_foundations/README.md) (principles + 6 questions) ·
[`damage_curve_implementation/`](damage_curve_implementation/) (global method + 3 cells) ·
