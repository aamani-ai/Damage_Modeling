...
This addendum does not change hail × solar model behavior. It names the canonical runtime artifact and makes the evidence/metric gates machine-readable.
...
```text
source spine:
    PVEL/Kiwa public hail-stress breakage anchors for PV module glass breakage,
    with IEC baseline hail certification treated as a near-zero physical anchor.

chosen y-axis:
    PV module glass/cell replacement damage ratio, approximated by public glass-breakage probability.

chosen form:
    logistic P_break(D), because hail breakage is bounded, monotone, threshold-like,
    and public anchors are sparse.

sources demoted:
    field performance, stow guidance, and general severe-weather guidance inform mechanisms
    and adjustment direction but not universal numeric field-loss parameters.

non-canonical legacy artifact:
    any capex-weighted whole-asset blend is blocked for curated M3 use because it disagrees
    with the failure-unit grain and the dossier parameters.
```
...
|---|---|---|---|
| Unstowed fragile/default D50 and k | `curve_fit_shape` | `T2_public_lab_standard_or_physics` | Public lab anchors directly constrain breakage probabilities. |
| Hardened D50 and k | `curve_fit_shape` | `T3_engineering_proxy_or_adjacent_empirical` | Sparse public hardened evidence; tail extrapolated. |
| Stowed +8 mm horizontal shift | `conditioner_adjustment` | `T4_placeholder_or_expert_judgment` | Direction supported; magnitude not universally calibrated. |
| Stowed max_DR 0.90 | `boundary_or_cap` | `T4_placeholder_or_expert_judgment` | Placeholder cap pending stow-specific tests or claims. |
