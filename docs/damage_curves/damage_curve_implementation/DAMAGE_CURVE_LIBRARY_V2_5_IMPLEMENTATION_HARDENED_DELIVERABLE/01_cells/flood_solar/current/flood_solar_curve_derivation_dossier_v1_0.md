...
This addendum does not change flood × solar model behavior. It serializes the piecewise/state curves and makes the metric gates machine-readable.
...
```text
source spine:
    DOE/FEMP solar flood mechanisms and NEMA water-damaged electrical-equipment guidance.

chosen y-axis:
    failure-unit damage ratio for electrical/foundation/module units by local component exposure.

chosen form:
    piecewise-linear state/depth curves, because flood damage follows waterline / ingress states
    better than a smooth biological-style fragility curve.

sources demoted:
    USACE depth-percent methods support tabular/interpolated form but not solar-specific values;
    scour and duration/salinity sources support mechanisms but not a generic numeric curve.
```
...
|---|---|---|---|
| Local-depth transform `h_i=max(0,WSE-z_i_crit)` | `axis_bridge` | `T2_public_lab_standard_or_physics` | Component elevation/freeboard is source-supported. |
| Electrical depth-damage ordinates | `curve_fit_shape` | `T3_engineering_proxy_or_adjacent_empirical` | Mechanisms are source-supported; exact percentages are engineering parameterization. |
| Foundation/scour velocity proxy | `open_seam_placeholder` | `T4_placeholder_or_expert_judgment` | Site hydraulics/geotech dominate; generic curve is placeholder. |
| Enclosure/conduit adjustment fields | `conditioner_adjustment` | `T3_engineering_proxy_or_adjacent_empirical` | Mechanism source-supported; numeric form deferred. |
