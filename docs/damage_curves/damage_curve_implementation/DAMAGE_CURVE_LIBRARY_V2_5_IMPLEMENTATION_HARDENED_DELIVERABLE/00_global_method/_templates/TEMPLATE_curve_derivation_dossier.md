...
## <N>. Derivation rationale and parameter tier table
...
```text
source spine chosen:
    <which source or source family most constrains the curve>

sources adopted for mechanism/form:
    <sources used for mechanism, x-axis, curve form, but not numbers>

sources demoted or rejected:
    <why not used numerically>

resulting tier mix:
    <which parameters are T1/T2/T3/T4 and why>
```

### Per-parameter tier table

| Parameter | Value | Param role | Tier | Source ID(s) | Reasoning | Update trigger |
|---|---:|---|---|---|---|---|
| `<param>` | `<value>` | `curve_fit_shape` | `T2_public_lab_standard_or_physics` | `<source>` | `<why>` | `<trigger>` |

### Adjustment provenance

| Adjustment | Field | Form | Type | Tier | Source ID(s) | Reasoning | Open seam |
|---|---|---|---|---|---|---|---|
| `<adjustment_id>` | `<field>` | `<formula>` | `<type>` | `<tier>` | `<source>` | `<why this form>` | `<replacement evidence>` |
