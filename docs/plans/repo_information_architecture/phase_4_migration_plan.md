...
The first file move is the scope anchor, because it is a repo-level doc and already has a new `docs/scope/`
surface. Everything else remains index-only until the redirect and link-check pattern is stable. The one
exception is that `docs/source_drops/` may define empty placement surfaces for future raw ZIPs, extracted
source-review copies, and manifests; that is not a migration of existing source material.
...
docs/damage_curves/damage_curve_implementation/DAMAGE_CURVE_LIBRARY_V2_5_IMPLEMENTATION_HARDENED_DELIVERABLE/99_source_context/
notebooks/
data/
...
files = [
    ...
    Path('docs/source_drops/raw_zips/README.md'),
    Path('docs/source_drops/extracted/README.md'),
    Path('docs/source_drops/manifests/README.md'),
