...
| Plan | Status | What |
|---|---|---|
| [`hazard_asset_coverage/`](hazard_asset_coverage/README.md) | breadth complete: 10/10 structural, 5/10 runtime; three noncanonical v1 proposals, hail-wind docs r2, and TC-wind-solar docs r2 complete | Continue with `wildfire_wind`, one cell at a time without forcing a version promotion. |
...
| [`tropical_cyclone_wind_solar_completion/`](tropical_cyclone_wind_solar_completion/README.md) | deep-curation audit complete; numerical v2 not earned | Preserve the exact Perry screen, acquire a portable wind + architecture/state + same-unit economics cohort, then build v2 only after the evidence gates pass. |
