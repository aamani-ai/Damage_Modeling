...
`08` had to curve-building: reason out the *system* first, fill
...
> **Why a guide, not a table (per `08`'s precedent).** We did not
...
precision the model can't use.** Doc `08` already fixed the grain
...
or to move to the next reasoning topic (x-axis framing → `01`).

*Links:* `08 granularity` (sets the grain) · `01 emit` ·
`00 §5` · risk-metrics reference (basis choice) ·
