...
| `requirement` · `exclusive_group` · `is_default` | subsystem + component lookup | **the grammar of a valid asset (04x)** — `requirement` (`required`/`recommended`/`optional`); `exclusive_group` marks a mutually-exclusive *one-of* (e.g. `FIXED_MOUNT` ⊕ `TRACKER` in `MOUNTING_KIND`); `is_default` is the fallback member. This is the *menu's rules* — see `04x`. |
...
This is the **inventory + the rules** (the *menu*). The act of **picking / swapping / adding** one of these on a specific asset (and in a resiliency variant) is **Gap 2 (structural variant)** — still pending the 04x Q1 decision. The exhaustive per-field detail lives in the generated `substrate_catalog.generated.md`.
...
Per-field source labels are **field-level honest** (DD-067) — e.g. an array-area field is labelled `uspvdb_v3.0`, not a composite `uspvdb+eia860` default. The raw-source schemas are archived under `docs/data/raw_sources/`.

---

## See also
- [`subsystem_code_lookup` / `component_code_lookup`] — the runtime vocabulary (dev DB)
- workspace `governance` · `schema` · `architecture` · `decisions` (DD log)
- `dimension_design_principles` · `engineering_data_sources_map` · `substrate_field_decomposition` *(the workspace canon — governance/architecture/schema/decisions + these references — relocates into `reference/workspace/` as a tracked follow-up; see `design/docs_folder_cleanup.md`)*
- `docs/data/raw_sources/` — the upstream schema archive
